...
export default function DEX() {
  ...
  useEffect(() => {
    if (!swapAmountIn || swapAmountIn <= 0) {
      setSwapRate(null);
      setPriceImpact(null);
      return;
    }
    calculateSwapOutput(swapAmountIn);
  }, [swapAmountIn, swapFromToken, swapToToken]);
  ...
  const calculateSwapOutput = async () => {
    ...
    try {
      ...
      // --- Extra Analytics ---
      const outValue = parseFloat(ethers.formatUnits(amountOut, 18));
      const inValue = parseFloat(swapAmountIn);

      // Calculate rate
      const rate = outValue / inValue;
      setSwapRate(rate);

      // Calculate price impact (approximation)
      const reserveRatio =
        parseFloat(ethers.formatUnits(reserveOut, 18)) /
        parseFloat(ethers.formatUnits(reserveIn, 18));
      const idealOut = inValue * reserveRatio;
      const impact = ((idealOut - outValue) / idealOut) * 100;

      setPriceImpact(impact.toFixed(2));
      setImpactDirection(impact > 0 ? "down" : "up");
    } catch (error) {
  ...
  const handleLiquidityAmountAChange = (value) => {
    ...
    const poolKey = `${liquidityTokenA}-${liquidityTokenB}`;
    const reserveData =
      reserves[poolKey] || reserves[`${liquidityTokenB}-${liquidityTokenA}`];
    ...
    if (
      ...
      const reserveA = parseFloat(
        ethers.formatUnits(
          isDirectPair ? reserveData.reserveA : reserveData.reserveB,
          18
        )
      );
      const reserveB = parseFloat(
        ethers.formatUnits(
          isDirectPair ? reserveData.reserveB : reserveData.reserveA,
          18
        )
      );

      const amountB = (value * reserveB) / reserveA;
      setLiquidityAmountB(amountB.toFixed(6)); // auto-fill
    } else {
      setLiquidityAmountB("");
    }
  };
  ...
  const createPool = async () => {
    if (liquidityTokenA === liquidityTokenB) {
      ...
      toast.success("Please select two different tokens, token A liquidity = token B Liquidity");
      return;
    }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (value && !isNaN(value)) {
                      calculateSwapOutput(value); // instantly recalc on typing
                    } else {
                      setPriceImpact(null);
                      setSwapRate(null);
                    }
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleLiquidityAmountAChange(e.target.value)}
                placeholder="0.0"
                className="input-field text-white focus:text-black focus:bg-amber-300"
              />
              <p className="text-xs text-gray-300 mt-1">
